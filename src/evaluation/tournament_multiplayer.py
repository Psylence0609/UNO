...
class TournamentMultiPlayer:
    ...
    def run_matchup(self, agent_names):
        ...
        if self.num_players == 2:
            ...
            desc = f"{agent_names[0][:10]} vs {agent_names[1][:10]} vs {agent_names[2][:10]}"
        elif self.num_players == 4:
            desc = f"{agent_names[0][:8]} vs {agent_names[1][:8]} vs {agent_names[2][:8]} vs {agent_names[3][:8]}"
        else:
            desc = f"{self.num_players}-player matchup"
...
# Run 2-player tournament (default 1000 games per matchup)
python src/evaluation/tournament_multiplayer.py --players 2

# Run 3-player tournament with 500 games per matchup
...
- Save results to tournament_{n}player_results.json
- Skip already completed matchups if the file exists
